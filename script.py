...
#pyinstaller -F -n Win_Simple.exe --icon=Murasame.ico --hidden-import=winrt.windows.media.control --hidden-import=winrt.windows.foundation --hidden-import=win32gui --hidden-import=win32con --hidden-import=win32api --hidden-import=httpx --hidden-import=pydantic script.py
...
class Config(BaseModel):
    ...
    skipped_names: list[str] = [
        '',  # 空字符串
        '系统托盘溢出窗口。', '新通知', '任务切换', '快速设置', '通知中心', '操作中心', 
        '日期和时间信息', '网络连接', '电池信息', '搜索', '任务视图', '任务切换', 'Program Manager',  # 桌面组件
        'Flow.Launcher', 'Snipper - Snipaste', 'Paster - Snipaste'  # 其他程序
    ]
    """当窗口标题为其中任意一项时将不更新"""
    
    not_using_names: list[str] = [
        '启动', '「开始」菜单',  # 开始菜单
...
# 媒体信息显示模式: 'prefix' - 作为前缀添加到当前窗口名称, 'standalone' - 使用独立设备
media_info_mode: 'standalone'
# 独立设备模式下的设备ID (仅当 media_info_mode = 'standalone' 时有效)
media_device_id: 'media-device'
# 独立设备模式下的显示名称 (仅当 media_info_mode = 'standalone' 时有效)
media_device_show_name: '正在播放'
# 是否启用电源状态获取
battery_info_enabled: true
# --- Ciallo～(∠・ω< )⌒★ end
"""
        with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
            f.write(config_content)
        print("Ciallo文件创建成功！")
        print("请修改 Ciallo.yaml 后再启动程序！")
        print("Ciallo～(∠・ω< )⌒★")
        input("按任意键退出程序...")
        return True
...
def load_config() -> Config:
    ...
    try:
        ...
        for key, value in raw_config.items():
            lowercase_config[key.lower()] = value
        
        # 使用 Pydantic 解析配置，自动支持类型验证
        try:
            config = Config(**lowercase_config)
            return config
        except Exception as e:
            print(f"配置文件验证失败: {e}")
            print("使用默认配置继续运行...")
...
DEVICE_SHOW_NAME: str = config.device_show_name
CHECK_INTERVAL: int = config.check_interval
BYPASS_SAME_REQUEST: bool = config.bypass_same_request
ENCODING: str = config.encoding
SKIPPED_NAMES: list[str] = config.skipped_names
NOT_USING_NAMES: list[str] = config.not_using_names
REVERSE_APP_NAME: bool = config.reverse_app_name
MOUSE_IDLE_TIME: int = config.mouse_idle_time
MOUSE_MOVE_THRESHOLD: int = config.mouse_move_threshold
DEBUG: bool = config.debug
PROXY: str = config.proxy
MEDIA_INFO_ENABLED: bool = config.media_info_enabled
MEDIA_INFO_MODE: str = config.media_info_mode
MEDIA_DEVICE_ID: str = config.media_device_id
MEDIA_DEVICE_SHOW_NAME: str = config.media_device_show_name
BATTERY_INFO_ENABLED: bool = config.battery_info_enabled
